...
export const ProductDetailModal = ({ isOpen, onClose, product, allMaterials, onRefresh }: Props) => {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={product ? `Recipe: ${product.name}` : "Product Details"}>
      {!product ? (
        <p className="text-gray-500 italic text-center">Select a product to see details.</p> 
      ) : (
        <div className="space-y-8">
          {/* Form Section */}
          <div className="bg-white/40 p-6 rounded-2xl border border-white/60 shadow-sm">
            <h4 className="font-black text-xs text-purple-600 mb-4 uppercase tracking-widest">Add Raw Material</h4>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-1">
                <label className="text-[10px] font-bold text-gray-400 uppercase ml-1">Material</label>
                <select 
                  className="w-full border border-gray-200 rounded-xl p-3 text-sm bg-white/50 focus:ring-2 focus:ring-purple-500 outline-none transition-all"
                  value={selectedMatId}
            ...
            <button 
              ...
              className="w-full mt-6 bg-purple-600 text-white py-3 rounded-xl font-bold text-sm hover:bg-purple-700 transition-all shadow-lg shadow-purple-100 disabled:opacity-50 disabled:shadow-none active:scale-95"
            >
              Add to Recipe
            </button>
          </div>
          ...
          <div>
            <div className="flex justify-between items-center mb-4 px-1">
              <h4 className="font-black text-xs text-gray-400 uppercase tracking-widest">Current Ingredients</h4>
            ...
            <div className="space-y-2 max-h-60 overflow-y-auto pr-2 custom-scrollbar">
              {product.materials && product.materials.length > 0 ? (
                product.materials.map((m) => (
                  <div key={m.id} className="flex justify-between items-center p-4 bg-white/30 rounded-xl border border-white/40 hover:bg-white/50 transition-colors group">
                    <div className="flex flex-col">
                      <span className="font-bold text-gray-700">{m.rawMaterial.name}</span>
                      <span className="text-xs text-purple-500 font-medium">{m.requiredQuantity} units required</span>
                    </div>
                    <button 
                ...
                <div className="text-center py-8 border-2 border-dashed border-gray-100 rounded-2xl">
                   <p className="text-sm text-gray-400">No ingredients linked yet.</p>
                </div>
              )}
            </div>
          </div>
